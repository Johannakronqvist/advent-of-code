...
for (let i = 0; i < rearrangementProcedure.length; i++) {
  ...
  for (let moves = cratesToMove; moves >= 1; moves--) {
    let currentStack = stacks[moveFromStack];
    let crateToMove = currentStack[currentStack.length - moves];

    stacks[moveToStack].push(crateToMove);
  }

  for (let moves = 1; moves <= cratesToMove; moves++) {
    stacks[moveFromStack].pop();
  }
}
